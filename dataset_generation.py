...
def create(path):
    ...
    for sig_name, signal in zip(record.sig_name, record.p_signal.T):
        ...
        for i, (label, peak) in enumerate(zip(ann.symbol, ann.sample)):
            if label == '/':
                label = "\\"
            print('\r{} [{}/{}]'.format(sig_name,
                                        i + 1, len(ann.symbol)), end="")
            ...
            if np.all([left > 0, right < len(signal)]):
                one_dim_data_dir = osp.join(
                    output_dir, '1D', name, sig_name, label)
                two_dim_data_dir = osp.join(
                    output_dir, '2D', name, sig_name, label)
