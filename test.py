...
if __name__ == "__main__":
    ...
    # mit_records = sorted([w.replace(".dat", "") for w in dat_files])
    # results = np.zeros((len(mit_records), 5), dtype=int)
    # # for record in mit_records:

    # data
    # record = wfdb.rdrecord(mitdb_dir+'/'+"116")
    # ecg = None
    # mode = 128
    ...
    # mask_right = (peaks + mode // 2) < len(signal)
    # mask = mask_left & mask_right
    # peaks = peaks[mask]
    # # unfiltered_ecg = signal[:, 0]
    # # r_peaks = pan_tompkins_detector(signal)
    # plt.figure()
    # # plt.plot(peaks)
    # plt.plot(ecg[peaks])
    # plt.show()
    # for record in mit_records:
    sig, fields = wfdb.rdsamp(mitdb_dir+'/'+"116")
    unfiltered_ecg = sig[:, 0]
    r_peaks = pan_tompkins_detector(unfiltered_ecg)
    plt.figure()
    # plt.plot(unfiltered_ecg)
    plt.plot(unfiltered_ecg[r_peaks])
    plt.title('Detected L-peaks')
    plt.show()
